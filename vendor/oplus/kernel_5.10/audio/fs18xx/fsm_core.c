...
int fsm_swap_channel(fsm_dev_t *fsm_dev, int next_angle)
{
    ...
    pr_addr(info, "pos:%02X, CHS12:%d", fsm_dev->pos_mask, chs12);
...
void fsm_speaker_onn(void)
{
    ...
    if (cfg->speaker_on || cfg->force_mute)
        return;
    ...
    if (cfg->chk_err) {
        fsm_list_func(fsm_dev, fs18xx_check_status_reg);
    }
...
ssize_t fsm_get_reg(char *buf)
 {
     ...
     list_for_each_entry(fsm_dev, &fsm_dev_list, list) {
         reg_end = 0xCD;
         idx = 0;
         len += snprintf(buf+len, PAGE_SIZE-len, "addr:%02X\n", fsm_dev->i2c->addr);
         ret |= fsm_access_key(fsm_dev, 1);
         for (reg_addr = 0; reg_addr <= reg_end; reg_addr++) {
             if(reg_addr == 0x0C) reg_addr = 0x46;
             else if(reg_addr == 0x4A) reg_addr = 0x4D;
             else if(reg_addr == 0x50) reg_addr = 0x5B;
             else if(reg_addr == 0x6D) reg_addr = 0x96;
             else if(reg_addr == 0xB0) reg_addr = 0xC0;
             ret |= fsm_reg_read(fsm_dev, reg_addr, &value);
             len += snprintf(buf+len, PAGE_SIZE-len, "%02X:%04X ", reg_addr, value);
             idx++;
